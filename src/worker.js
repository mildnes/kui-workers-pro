...
function withWorkerOrigin(env, origin) {
    return Object.assign({}, env, { PAGES_ORIGIN: origin, REALTIME_URL: origin });
}
...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            const rayId = request.headers.get('cf-ray') || `local-${Date.now()}`;
            console.error(`[worker] uncaught fetch error ray=${rayId}`, error?.stack || error);
            return new Response(`KUI Worker request failed (${rayId})`, {
                status: 500,
